...
namespace GICutscenes
{
    ...
    internal class Demuxer
    {
        ...
        private static (ulong, bool)? EncryptionKeyInBLK(string videoFilename)
        {
            ...
            if (!File.Exists(versionsFilePath))
                throw new FileNotFoundException(
                    "File versions.json couldn't be found in the folder of the tool."
                );
            videoFilename = Path.GetFileNameWithoutExtension(videoFilename);
            string jsonString = File.ReadAllText(versionsFilePath);
            VersionList? versions = JsonSerializer.Deserialize<VersionList>(
                jsonString,
                VersionJson.Default.VersionList
            );
            if (versions?.list == null)
                throw new JsonException(
                    "Json content from versions.json is invalid or couldn't be parsed..."
                );
            Version? v = Array.Find(
                versions.list,
                x =>
                    (x.videos != null && x.videos.Contains(videoFilename))
                    || (
                        x.videoGroups != null
                        && Array.Exists(
                            x.videoGroups,
                            y => y.videos != null && y.videos.Contains(videoFilename)
                        )
                    )
            );
            if (v == null)
            {
                ...
                Console.WriteLine(
                    "Unable to find the second key in versions.json for " + videoFilename
                );
                Console.ResetColor();
                return null;
            }
        ...
        {
            if (!file.Exists)
                throw new FileNotFoundException($"File {file.Name} does not exist.");
            if (!output.Exists)
                throw new DirectoryNotFoundException(
                    $"Directory {output.FullName} does not exist."
                );
            ...
            USM usmFile = new(file, key1, key2);
            ...
            for (int i = 0; i < decodingTasks.Length; i++)
            {
                ...
                decodingTasks[i] = Task.Run(() =>
                {
                    ...
                    audioFile.ConvertToWAV(output.FullName);
                });
            }
